...
```cpp
//强行调用基类中定义的函数版本而不管 baseP 的动态类型是什么double undiscounted = baseP->Quote::net_price(42);
```
...
```cpp
struct Base{	Base() : mem(0){}protected:    int mem;};struct Derived : Base{    Derived(int i) : mem(i){}    int get_mem() {return mem;}protected:    int mem;}Derived d(42);cout << d.get_mem() << endl;//42
```

**3. 通过作用域运算符使用隐藏的成员**

```cpp
struct Derived : Base{    int get_mem() {return Base::mem;}}//0
```
...
```cpp
struct Base{	int memfcn(); };struct Derived : Base{	int memfcn(int);  };Derived d;Base b;b.memfcn();		// 调用Base::memfcnd.memfcn(10);	// 调用Derived::memfcn//d.memfcn();d.Base::memfcn();// 调用Base::memfcn
```
